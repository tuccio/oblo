    ...
    bool importer::finalize_import(asset_registry& registry, const std::filesystem::path& destination)
    {
        ...
        for (const pending_asset_import& assetImport : m_assets)
        {
            ...
            for (const import_artifact& artifact : assetImport.artifacts)
            {
                ...
                if (!registry.save_artifact(artifact.id, artifact.data.get_type(), artifactPtr))
                {
                    OBLO_ASSERT(false); // TODO: Log?
                    allSucceeded = false;
                    continue;
                ...
                assetMeta.artifacts.emplace_back(std::move(artifactIt->second));
            }

            allSucceeded &= registry.save_asset(mainArtifact.id, destination, assetFileName, std::move(assetMeta));
        }
