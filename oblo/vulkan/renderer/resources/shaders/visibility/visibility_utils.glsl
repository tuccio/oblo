...
vec2 visibility_calculate_last_frame_position_ndc_2d(in mat4 lastFrameViewProjection, in vec3 prevPositionWS)
{
...
vec2 visibility_calculate_last_frame_position_ndc_2d(
    in mat4 lastFrameViewProjection, in barycentric_coords bc, in triangle prevTriangleWS)
{
    const vec3 prevPositionWS = barycentric_interpolate(bc, prevTriangleWS.v);
    return visibility_calculate_last_frame_position_ndc_2d(lastFrameViewProjection, prevPositionWS);
}

vec3 visibility_calculate_last_frame_position_ndc_3d(in mat4 lastFrameViewProjection, in vec3 prevPositionWS)
{
    const vec4 prevPositionCS = lastFrameViewProjection * vec4(prevPositionWS, 1);
