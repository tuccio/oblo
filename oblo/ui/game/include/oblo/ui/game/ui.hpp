...
namespace oblo::ui::game
{
    ...
    using oblo::vec4;
    using oblo::ui::color;
    using oblo::ui::layout_id;
    using oblo::ui::layout_state;
    using oblo::ui::rect;
    ...
    using measure_text_fn = function_ref<vec2(const char* text, f32 fontHeight)>;
    ...
    public:
        ...
        void begin_frame(std::span<const input_event> events, time dt, vec2 layoutSize);
        void end_frame();
        ...
        std::span<const layout_element> elements() const
        {
            return get_elements(*m_layout);
        }

        vec2 measure(const char* text, f32 fontHeight) const;
        ...
        void emit_rect(
            layout_id id, const color& fill, f32 cornerRadius, const rect& local = rect{0.f, 0.f, -1.f, -1.f});
        ...
        std::span<const draw_text> texts() const
        {
            return m_texts;
        }
