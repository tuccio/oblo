...
namespace oblo
{
    ...
    expected<> entity_hierarchy::load(const data_document& doc)
    {
        const auto propertyRegistries = module_manager::get().find_services<const property_registry>();
    ...
    expected<> entity_hierarchy::copy_from(const ecs::entity_registry& other, const ecs_serializer::write_config& cfg)
    {
        if (&other == &m_registry)
        {
            return unspecified_error;
        ...
        if (!ecs_serializer::write(doc, doc.get_root(), other, *propertyRegistries.front(), cfg))
        {
            return unspecified_error;
        }
